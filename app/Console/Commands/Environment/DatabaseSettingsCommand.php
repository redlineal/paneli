...
namespace Amghost\Console\Commands\Environment;

use PDOException;
use Illuminate\Console\Command;
use Illuminate\Contracts\Console\Kernel;
use Illuminate\Database\DatabaseManager;
use Amghost\Traits\Commands\EnvironmentWriterTrait;
...
class DatabaseSettingsCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->variables['DB_USERNAME'] = $this->option('username') ?? $this->ask(
            trans('command/messages.environment.database.username'), $this->config->get('database.connections.mysql.username', 'amghost')
        );
        ...
        try {
            ...
            if ($this->confirm(trans('command/messages.environment.database.try_again'))) {
                $this->database->disconnect('_amghost_command_test');
    ...
    {
        $this->config->set('database.connections._amghost_command_test', [
           'driver' => 'mysql',
        ...
        $this->database->connection('_amghost_command_test')->getPdo();
    }
}
