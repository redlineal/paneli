...
@section('footer-scripts')
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            ...
            $.ajax({
                method: 'DELETE',
                url: Router.route('admin.nodes.view.allocation.removeSingle', { node: Amghost.node.id, allocation: allocation }),
    ...
    function sendAlias(element) {
        ...
        $.ajax({
            method: 'POST',
            url: Router.route('admin.nodes.view.allocation.setAlias', { id: Amghost.node.id }),
            headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') },
            data: {
                alias: element.val(),
