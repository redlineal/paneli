...
@section('footer-scripts')
    ...
    $(document).ready(function () {
        ...
        $('#pEggId').select2({placeholder: 'Select a Nest Egg'}).on('change', function () {
            ...
            var parentChain = _.get(Amghost.nests, $("#pNestId").val());
            ...
            if (objectChain.id === parseInt(Amghost.server.egg_id)) {
                $('#pDockerImage').val(Amghost.server.image);
            }
            ...
            if (Amghost.server.pack_id !== null) {
                $('#pPackId').val(Amghost.server.pack_id);
            }

            $('#appendVariablesTo').html('');
            $.each(_.get(objectChain, 'variables', []), function (i, item) {
                var setValue = _.get(Amghost.server_variables, item.env_variable, item.default_value);
        ...
        $('#pNestId').select2({placeholder: 'Select a Nest'}).on('change', function () {
            $('#pEggId').html('').select2({
                data: $.map(_.get(Amghost.nests, $(this).val() + '.eggs', []), function (item) {
                    return {
                        id: item.id,
                        text: item.name,
                    };
            ...
            if (_.isObject(_.get(Amghost.nests, $(this).val() + '.eggs.' + Amghost.server.egg_id))) {
                $('#pEggId').val(Amghost.server.egg_id);
            }
