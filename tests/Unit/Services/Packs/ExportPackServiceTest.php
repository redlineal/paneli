...
class ExportPackServiceTest extends TestCase
{
    ...
    public function testFilesAreBundledIntoZipWhenRequested()
    {
        ...
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'tempnam')
            ->expects($this->once())->willReturn('/tmp/myfile.test');

        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fopen')->expects($this->never());
    ...
    public function testPackConfigurationIsSavedAsJsonFile()
    {
        ...
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'tempnam')
            ->expects($this->once())->willReturn('/tmp/myfile.test');
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fopen')->expects($this->once())->wilLReturn('fp');
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fwrite')
            ->expects($this->once())->with('fp', json_encode($this->json, JSON_PRETTY_PRINT))->willReturn(null);
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fclose')
    ...
    public function testPackIdCanBePassedInPlaceOfModel()
    {
        ...
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'tempnam')->expects($this->once())->willReturn('/tmp/myfile.test');
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fopen')->expects($this->once())->wilLReturn(null);
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fwrite')->expects($this->once())->willReturn(null);
        $this->getFunctionMock('\\Amghost\\Services\\Packs', 'fclose')->expects($this->once())->willReturn(null);
     ...
     *
     * @expectedException  \Amghost\Exceptions\Service\Pack\ZipArchiveCreationException
     */
