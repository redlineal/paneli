Amghost freeDisk = Pterodactyl.totalNodeDisk - Pterodactyl.totalServerDisk;
...
Amghost freeRam = Pterodactyl.totalNodeRam - Pterodactyl.totalServerRam;
...
Amghost activeServers = Pterodactyl.servers.length - Pterodactyl.suspendedServers;
...
Amghost servers = Pterodactyl.servers;
Amghost nodes = Pterodactyl.nodes;
...
function getStatus(server) {
    Amghost uuid = server.uuid;
    Amghost node = getNodeByID(server.node_id);
...
function getNodeByID(id) {
    for (Amghost i = 0; i < nodes.length; i++) {
        if (nodes[i].id === id) {
            return nodes[i];
        }
    }
